...
class Plasticity(Problem):
    ...
    def custom_init(self):
        ...
        self.sig0 = 0.0  # Initial value for sig0
        self.hardening_modulus = 0.0  # Initial value for the hardening modulus
        self.internal_vars = [self.sigmas_old, self.epsilons_old, self.sig0, self.plastic_strain]

    def set_material_properties(self, E, nu, sig0, hardening_modulus):
        self.E = E
        self.nu = nu
        self.sig0 = sig0
    ...
    def get_maps(self, return_all=False):
        ...
        def stress_return_map(u_grad, sigma_old, epsilon_old, sig0, plastic_strain):
            ...
            # Isotropic hardening
            sig0 += self.hardening_modulus * equivalent_plastic_strain
            ...
            updated_plastic_strain = plastic_strain + safe_divide(f_yield_plus, (2.0 * self.hardening_modulus))
...
def run_model(E, sig0, filename, plotname):
    ...
    disps = np.hstack((np.linspace(0., 0.04, 11), np.linspace(0.035, 0., 10)))
    ...
    problem.set_material_properties(E=E, nu=0.3, sig0=sig0, hardening_modulus=0)
    avg_stresses = []
    avg_strains = []
    for i, disp in enumerate(disps):
        ...
        avg_strains.append(avg_strains)
...
if __name__ == "__main__":
    E_range = [13e3, 15e3]
    E_mu = sum(E_range) / 2
    E_std = (max(E_range) - min(E_range))/6 
    E_dist = norm(loc=E_mu, scale=E_std)
    sig0_range = [8, 14]
    sig0_mu = sum(sig0_range) / 2
    sig0_std = (max(sig0_range) - min(sig0_range))/6 
    sig0_dist = norm(loc=sig0_mu, scale=sig0_std)
    
    N = 100
    E_ = E_dist.rvs(N)
    sig0_ = sig0_dist.rvs(N)

    for i in range(N):
        tag = str(i+1).zfill(3)
        filename = f"output_{tag}"
        plotname = f"output_{tag}"
        run_model(E_[i], sig0_[i], filename, plotname)
