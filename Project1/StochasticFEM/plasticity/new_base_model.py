...
class Plasticity(Problem):
    ...
    def get_maps(self, return_all=False):
        ...
        def stress_return_map(u_grad, sigma_old, epsilon_old, sig0, plastic_strain):#, alpha):
            epsilon_crt = strain(u_grad)
            epsilon_inc = epsilon_crt - epsilon_old
            sigma_trial = stress(epsilon_inc) + sigma_old
            s_dev = sigma_trial - safe_divide(1., self.dim * \
                                        np.trace(sigma_trial) * np.eye(self.dim))
            s_norm = safe_sqrt(3. / 2. * np.sum(s_dev * s_dev))
            
            initial_equivalent_plastic_strain = safe_sqrt((2./3.) * \
                                                    np.sum(plastic_strain**2))
            
            sig0 += self.Q_inf * (1 - np.exp(-self.b * initial_equivalent_plastic_strain)) 

            f_yield = s_norm - sig0
            f_yield_plus = np.where(f_yield > 0., f_yield, 0.)

            plastic_strain_rate = safe_divide(3., 2*self.E) * f_yield_plus
            dt = 1e3*(1/15)
            delta_equivalent_plastic_strain = plastic_strain_rate * dt
            updated_plastic_strain = plastic_strain + plastic_strain_rate*dt 
...
def run_model(E, sig0, filename, plotname):
    ...
    disps = np.hstack((np.linspace(0., 4, 15)))#, np.linspace(0.13, 0., 8)))
